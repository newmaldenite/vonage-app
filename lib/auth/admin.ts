...
const ADMIN_EMAIL_DOMAIN = process.env.ADMIN_EMAIL_DOMAIN || "https://vonage-app-three.vercel.app/sign-in";
...
export async function sendAdminMagicLink( email:string ) {
    ...
    if ( !email.endsWith(`@${ADMIN_EMAIL_DOMAIN}`)) {
        throw new Error(`Admin email must be a valid ${ADMIN_EMAIL_DOMAIN} address`)
    }
    ...
    const { error } = await supabase.auth.signInWithOtp({ // pulling the error object out from the result of signInWithOtp()
        email,
        options: {
